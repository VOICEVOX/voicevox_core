...
pub(crate) fn into_result_code_with_error(result: CApiResult<()>) -> VoicevoxResultCode {
    ...
    fn into_result_code(result: CApiResult<()>) -> VoicevoxResultCode {
        ...
        match result {
            ...
            Err(RustApi(UserDictRead(_))) => VOICEVOX_USER_DICT_READ_ERROR,
            Err(RustApi(UserDictWrite(_))) => VOICEVOX_USER_DICT_WRITE_ERROR,
            Err(RustApi(WordNotFound(_))) => VOICEVOX_WORD_NOT_FOUND_ERROR,
            Err(RustApi(UserDictLoad(_))) => VOICEVOX_USER_DICT_LOAD_ERROR,
