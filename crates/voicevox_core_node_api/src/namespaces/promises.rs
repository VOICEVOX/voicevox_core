...
pub mod promises {
    ...
    impl JsOpenJtalk {
        ...
        pub async fn create(open_jtalk_dict_dir: String) -> Result<JsOpenJtalk> {
            Ok(JsOpenJtalk {
                open_jtalk: convert_result(OpenJtalk::new(open_jtalk_dict_dir).await)?,
            })
        }
        ...
        pub async fn use_user_dict(&self, user_dict: &JsUserDict) -> Result<()> {
            convert_result(self.open_jtalk.use_user_dict(&user_dict.user_dict).await)
        }
    }
    ...
    impl JsUserDict {
        ...
        pub async fn load(&self, store_path: String) -> Result<()> {
            convert_result(self.user_dict.load(&store_path).await)
        }
        ...
        pub async fn save(&self, store_path: String) -> Result<()> {
            convert_result(self.user_dict.save(&store_path).await)
        }
    }
    ...
    impl JsSynthesizer {
        ...
        pub async fn load_voice_model(&self, model: &JsVoiceModel) -> Result<()> {
            convert_result(self.synthesizer.load_voice_model(&model.voice_model).await)
        }
        ...
        pub async fn from_path(path: String) -> Result<JsVoiceModel> {
            convert_result(
                VoiceModel::from_path(&path)
                    .await
                    .map(|voice_model| JsVoiceModel { voice_model }),
            )
        }
