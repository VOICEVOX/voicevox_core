...
`Score`中の[`Note`]には無音のノートと無音でないノートがあります。[`Score.notes`]の最初のノートは無音にします。

`Note`には次の値を設定します。

- [`Note.frame_length`]: 秒数に93.75をかけ、端数を調整したものを設定（例: 125BPM (_**B**eats **P**er **M**inute_)における一拍として、(125 / 60)⁻¹ × 93.75 = `45`）。
- 無音のノートの場合:
    - [`Note.lyric`]\: 空文字列を指定。
